...
u8 halsig_gerr(void)
{
    ...
    dval_hall = (s16)(et_hall.hcnt_sub) - (s16)(et_hall.hall_cnt);
    if ((dval_hall > 200) || (dval_hall < -200))
    {
        return ERR_HAL_SUBS;
    }

    if (et_hall.herr_sub >= 5)
    {
        et_hall.herr_sub = 0;
        return ERR_HAL_MUTX;
    }
...
u8 main_hal_get(void){
  return (GPIO_PB_IDR & 0x20) ? 0 : 1; 
}

u8 ref_hal_get(void){
  return (GPIO_PB_IDR & 0x10) ? 0 : 1;
}