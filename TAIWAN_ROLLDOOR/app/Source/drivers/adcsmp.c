 ...
 VODET adcsmp_init(VODET)
{
  ...
  for (index_1 = 0; index_1 < ADC_CH_MAX; ++index_1)
  {
    ...
    for (index_2 = 0; index_2 < ADC_FILT_MAX; ++index_2) et_adcs[index_1].value[index_2] = 0;
  }
...
VODET adcsmp_task(VODET)
{
  ...
  if ((0 == et_adcs[chan].ready) && ((ADC_FILT_MAX - 1) == et_adcs[chan].index)) et_adcs[chan].ready = 1;
  ...
  for (index = 1; index < ADC_FILT_MAX; index++) {
    if (et_adcs[chan].value[index] < min) min = et_adcs[chan].value[index];
    if (et_adcs[chan].value[index] > max) max = et_adcs[chan].value[index];
  ...
  if (0 == et_adcs[chan].ready) et_adcs[chan].result = 0;
  /* select new channel */
