...
void sys_read_eep(void){
    ...
    wdtdog_feed();
    if ((glb_ctrl.posval_top >= glb_ctrl.posval_bot) /*|| (glb_ctrl.posval_bot > POS_VAL_MAX)*/) {
      #ifdef UART_DEBUG
        PutString("--pos error !!\r\n ");
    	PutString("postop : ");
...
void sys_train_dn(void){
    ...
    PosSmp  = (glb_ctrl.posval_bot - glb_ctrl.posval_top + DN_FORCE_SAMP -1)/DN_FORCE_SAMP;
    ...
    /* run until top side */
	    ...
	    BUZ_ctrl(0, 0);
	    glb_ctrl.dnForce[(glb_ctrl.posval_bot - glb_ctrl.posval_top) / PosSmp] = adcValue;
            motors_pwms(0);
            sys_ctrl.sub_sts = 3;
            sys_ctrl.delay0 = 100;
	    return;
        ...
        if ((CurPosition >= glb_ctrl.posval_top) && (CurPosition <= glb_ctrl.posval_bot)) {
...
void sys_train_up(void){
    ...
    CurPosition = pos_get();
    adcValue = (U08ET)(getadc_data(ADC_CH_MOTOR) >> 2);
    PosSmp  = (glb_ctrl.posval_bot - glb_ctrl.posval_top + UP_FORCE_SAMP - 1)/UP_FORCE_SAMP;
    ...
    /* run until top side */
	    ...
	    glb_ctrl.upForce[(glb_ctrl.posval_bot - glb_ctrl.posval_top)/PosSmp] = adcValue;
	    BUZ_ctrl(0, 0);
            motors_pwms(0);
            sys_ctrl.sub_sts = 3;
            sys_ctrl.delay0 = 100;
...
void sys_stay_stop(void){
    ...
    if ((ET_TRUE == remote_hits() || KEY_ST_HIGH == getkey_stabe(KEY_ID_K5)) && (glb_ctrl.posval_top != POS_ERR && glb_ctrl.posval_bot != POS_ERR)) {
        if (MOT_CT_DOWN == sys_ctrl.lst_dir) { //UP   control
            if ( CurPosition < glb_ctrl.posval_top + glb_ctrl.inertia_top) { //pos == top 
	        #ifdef UART_DEBUG
	    	PutString("fl-1\r\n"); 	
		#endif
                sys_ctrl.sys_sts = SYS_ST_EXCUTE_DN;
	    	...
	    	PutString("fl-3\r\n"); 	
            	...
            	if (CurPosition > glb_ctrl.posval_bot - glb_ctrl.inertia_bot) { //bot sensor
		        #ifdef UART_DEBUG
	    	        PutString("fl-5\r\n"); 	
		        #endif
	                sys_ctrl.sys_sts = SYS_ST_EXCUTE_UP;
	...
	/* normal route overforce judge */
        if (Position > glb_ctrl.posval_top + HAL_CNT_SLOW_STOP) {
            ...
            if (sys_ctrl.delay0 > 0) {
                ...
                if (Current > (glb_ctrl.uforce_max + bk_force[glb_ctrl.force_lv - 1][3])) {
//			sys_ctrl.OvCurCount++;
			...
			sys_ctrl.delay0 = 200;
                	return;
                }/*else if (Current < (calcForce+(bk_force[glb_ctrl.force_lv - 1][3]/2))){
		    if (sys_ctrl.OvCurCount > 0)
			sys_ctrl.OvCurCount--;	
		}*/
            }
            else {
                /* Resistance rebound in normal duration */
                calcForce = max(glb_ctrl.upForce[index-1], glb_ctrl.upForce[index], glb_ctrl.upForce[index+1]);
...
//			sys_ctrl.OvCurCount++;
			...
			sys_ctrl.delay0 = 200;
                	return;
                }/*else if (Current < (calcForce+(bk_force[glb_ctrl.force_lv - 1][4]/2))){
		    if (sys_ctrl.OvCurCount > 0)
			sys_ctrl.OvCurCount--;	
		}*/
            }
...
//		    sys_ctrl.OvCurCount++;
		    ...
		    return;
//		    sys_ctrl.OvCurCount++;
                }/*else if (Current < (calcForce+(bk_force[glb_ctrl.force_lv - 1][5]/2))){
		    if (sys_ctrl.OvCurCount > 0)
			sys_ctrl.OvCurCount--;
		}*/
            }
...
void sys_excute_dn(void){
	...
	if (index == ((glb_ctrl.posval_bot - glb_ctrl.posval_top) / PosSmp))    
        ...
        if (Position < glb_ctrl.posval_bot - HAL_CNT_SLOW_STOP) {
            ...
            if (sys_ctrl.delay0 > 0) {
                sys_ctrl.delay0--;
                if (Current > (glb_ctrl.dforce_max + bk_force[glb_ctrl.force_lv - 1][0])) {
		        sys_ctrl.sys_sts = SYS_ST_EXCUTE_BK;
			BUZ_ctrl(EXCUTE_AB_PERIOD, EXCUTE_AB_TIMES);
		   	sys_ctrl.sub_sts = 0;
		        return;
                ...
                calcForce = max(glb_ctrl.dnForce[index-1], glb_ctrl.dnForce[index], glb_ctrl.dnForce[index+1]);
...
/*        if((Postition < sys_ctrl.ErrPrePosition) && ((sys_ctrl.ErrPrePosition - Postition) > 4)){
    	    if (((Postition < sys_ctrl.ErrPrePosition) && ((sys_ctrl.ErrPrePosition - Postition) > 15)) ||\
       		((Postition > sys_ctrl.ErrPrePosition) && ((Postition - sys_ctrl.ErrPrePosition) > 15)) ){
		    sys_ctrl.EncoderErrCnt += 1300;  
	    } else {
		    sys_ctrl.EncoderErrCnt = 0;
	            sys_ctrl.ErrPrePosition = Postition;
	    }*/
    
    if ((MOT_CT_DOWN == sys_ctrl.cur_dir) && (SYS_ST_LIMIT_ST != sys_ctrl.sys_sts)){
        if((Postition > sys_ctrl.ErrPrePosition) && ((Postition - sys_ctrl.ErrPrePosition) > 4)){
    	    sys_ctrl.EncoderErrCnt = 0;
            sys_ctrl.ErrPrePosition = Postition;
	} else {
	    if (sys_ctrl.start_count < 500)
    ...
    } else if ((MOT_CT_UP == sys_ctrl.cur_dir) && (SYS_ST_LIMIT_ST != sys_ctrl.sys_sts)){
        if((Postition < sys_ctrl.ErrPrePosition) && ((sys_ctrl.ErrPrePosition - Postition) > 4)){
    	    sys_ctrl.EncoderErrCnt = 0;
            sys_ctrl.ErrPrePosition = Postition;
	} else {
