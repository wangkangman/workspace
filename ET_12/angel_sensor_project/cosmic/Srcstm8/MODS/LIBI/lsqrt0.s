...
;
_lsqrt:
	ld	a,(3,sp)	; test MSW hi
	jrne	nlow		; not zero, set mask
	ld	a,(4,sp)	; test MSW lo
	jrne	nint		; not zero, set mask
	ldw	x,(5,sp)	; set input value
	jp	_isqrt		; and compute result
nint:
...
bcl1:
	...
	incw	x
inl1:
	ld	a,(tabhi,x)	; find nearest value
	cp	a,c_x
	jrugt	bcl1		; loop while too low
	ld	a,(tabhi+1,x)	; read starting mask
	ld	c_x,a		; save value
	clr	c_x+1
	ld	c_y,a		; save mask
	clr	c_y+1
