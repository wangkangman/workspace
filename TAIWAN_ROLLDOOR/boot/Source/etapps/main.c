...
VODET sys_wait_evt(VODET)
{
    ...
    if( SYS_ST_SUB0 == sys_ctrl.sub_sts ) {
        ...
        /*  FF 01 00 05 11  */    
        if (ET_TRUE == uartct_recv(pbuf, 5)) {
            if (0x11 == pbuf[4]){
                ledtsk_ctrl(LED_ID_REMOTE, LED_ST_ON, 0);
    ...
    if( SYS_ST_SUB1 == sys_ctrl.sub_sts ) {
        if( KEY_ST_HIGH == getkey_stabe(KEY_ID_REMOTE) ) {
    ...
    /* check if K1 keep press for 1 second */
        ...
        sys_ctrl.delay0--;
        if( KEY_ST_LOW == getkey_stabe(KEY_ID_REMOTE) ) {
            sys_ctrl.sys_sts = SYS_ST_PROG_END;
            sys_ctrl.sub_sts = SYS_ST_SUB1;
        }
        if( 0 == sys_ctrl.delay0 ) {
...
VODET sys_file_get(VODET)
{
    ...
    ledtsk_ctrl(LED_ID_REMOTE, led_sts[sys_ctrl.dis_sts][0], 0);
