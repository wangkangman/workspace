...
VODET motors_init(VODET)
{
    ...
    GPIO_PB_DDR |=  (1<<1);  /* direction output */
    GPIO_PB_CR1 |=  (1<<1);  /* push-pull        */
    GPIO_PB_CR2 &= ~(1<<1);  /* max speed 2MHz   */
...
{
   	#if DBG_LV_CURRENT & DBG_LV_PROCESS   	
		printf("<motors_ctrl> motor deriction :"); 
		if(ctrl == MOT_CTS_UPUP)
			printf("UP"); 
		else if(ctrl == MOT_CTS_DOWN)
			printf("DOWN");
		else
			printf("STOP");
		printf("\r\n\r\n");	
    ...
    switch(ctrl) {
        //case MOT_CTS_UPUP:
				case MOT_CTS_DOWN:
//        	  if (KEY_ST_HIGH == getkey_curst(KEY_ID_LR)) 
						if(KEY_ST_HIGH == glb_ctrl.LorR_Set) {
                GPIO_PD_ODR |=  (1<<2);     
                GPIO_PB_ODR &= ~(1<<1);     /*M-  DA2 actuating UP*/
            } else {
                GPIO_PD_ODR &= ~(1<<2);     /*M+  DA1 actuating DOWN*/
                GPIO_PB_ODR |=  (1<<1);     
            }
            break;
...
VODET halsig_samp(VODET)
{
    ...
    {
        if ((ET_TRUE == et_hall.herr_ena) && \
        	  (et_hall.herr_cnt < et_hall.herr_max)) {
            et_hall.herr_cnt++;
        }
    }
}
