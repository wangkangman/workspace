    ...
    if (res == 0) {
      ...
      if (res == 0)                                 //打开目录成功
      {
        ...
        while (res == 0) //开始复制文件夹里面的东东
        {
          ...
          fn = (u8 *)(*finfo->lfname ? finfo->lfname
                                     : finfo->fname); //得到文件名

          dstpathlen =
              strlen((const char *)dstpathname); //得到当前目标路径的长度
              ...
              0X10) //是子目录(文件属性,0X20,归档文件;0X10,子目录;)
          {
            ...
            res = exf_fdcopy(fcpymsg, srcpathname, dstpathname, totsize,
                             cpdsize, fwmode); //拷贝文件夹
          } else                               //非目录
          {
            ...
            res = exf_copy(fcpymsg, srcpathname, dstpathname, *totsize,
                           *cpdsize, fwmode); //复制文件
