...
/**
  ...
  * V0.1.4: infra excute back, delay more time, origin pwm down;
  *
  *
  ******************************************************************************
  */
...
void sys_excute_close(void) {
  ...
  /* run until top side */
    ...
    if (hallCnt < sys_ctrl.hall_dest - HAL_CNT_SLOW_STOP) {
      ...
      if (sys_ctrl.delay0 > 0) {
        ...
        if (adcValue > force_res) {
          ...
          sys_ctrl.excute_back_type = BACK_FORCE;
          sys_ctrl.sub_sts = 0;
          return;
        }
      } else {
    ...
    if (ET_TRUE == infra_status) {
      sys_ctrl.sys_sts = SYS_ST_EXCUTE_BACK;
          sys_ctrl.excute_back_type = BACK_INFRA;
      sys_ctrl.sub_sts = 0;
      return;
    }
...
void sys_excute_back(void) {
  ...
  else if (1 == sys_ctrl.sub_sts)
  {
    ...
    if (0 == sys_ctrl.delay0) {
      motors_ctrl(MOT_CTS_STOP);
      sys_ctrl.sub_sts = 2;
      sys_ctrl.delay0 = (sys_ctrl.excute_back_type == BACK_FORCE)
                        ? 60
                        : 200;
    }
  }
  ...
  else if (3 == sys_ctrl.sub_sts)
  {
    ...
    if (0 == sys_ctrl.delay0)
    {
      ...
      sys_ctrl.cur_pwm = (sys_ctrl.excute_back_type == BACK_FORCE)
                        ? PWM_VAL_DFT_REV
                        : PWM_VAL_MIN_ALL;
      sys_ctrl.sys_sts = SYS_ST_EXCUTE_OPEN;
...
VODET WiFi_process(VODET) {
  ...
  hallCnt      = halsig_gcnt();
  /* ===============================================================================
     ------------------judge if GD50 connected with wifi
     adapter--------------------
     ===============================================================================*/
  ...
  /* ===============================================================================
    ...
    /*----------write command to sys_ctrl, then processed by
     * main_func-----------*/
